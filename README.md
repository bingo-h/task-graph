...
- **外观自定义**：配色方案（内置预设或自定义 TOML 文件）、深浅模式、全局圆角、界面风格（扁平/新拟态）均可在设置里独立切换，详见下方[外观与自定义配色](#外观与自定义配色)
...
- **界面风格**：扁平 / 新拟态二选一，可随时切换——只影响顶栏、侧栏、右键菜单、弹窗、统计卡片和按钮等界面"外壳"，不影响 DAG 图任务节点的状态色（完成/超时/今日/锁定等语义色永远保持扁平、清晰可辨）
